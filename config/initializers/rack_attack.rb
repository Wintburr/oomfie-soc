...
class Rack::Attack
  ...
  throttle('throttle_per_token_api', limit: 900, period: 5.minutes) do |req|
  ...
  throttle('throttle_api_media', limit: 120, period: 30.minutes) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 120, period: 10.minutes) do |req|
  ...
  throttle('throttle_api_delete', limit: 3000, period: 30.minutes) do |req|
